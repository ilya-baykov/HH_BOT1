...
from src.neural_gateway.neural_gateway_client import model_gpt_4o_mini
from src.neural_gateway.prompt_generator import PromptGenerator
from src.pdf_parser.download_pdf import PDFDownloader
from src.pdf_parser.pdf_reader import PDFTextExtractor
from src.pdf_parser.text_writer import TextWriter
...
from src.resumes_handler.json_extractor import JsonExtractor
...
class ResumesHandler:
    ...
    def resume_process_no_limits(self, recruiter_params: RecruiterDataParams) -> list[Applicant]:
        applicants = []
        report_database.connect()  # Связываемся с отчётной базой данных
        for resume in self.resumes_list:
            fetched_resume_id = db_operations.fetch_data(db=report_database, table_name="public.resume_reports",
                                                         column_name='resume_id',
                                                         value=resume.get('id'), auto_close=False)
            if fetched_resume_id:  # Это резюме уже есть в базе данных
                logger.debug("Резюме уже есть в БД")
                continue
            try:
                ...
                applicant = Applicant(
                    ...
                    pdf_path=None, total_experience=None, skills=None
                )
    ...
    def resume_process_with_limits(sorted_applicants: list[Applicant], recruiter_params: RecruiterDataParams):
        ...
        for applicant in sorted_applicants:  # Обходим список потенциальных кандидатов
            ...
            pdf_downloader = PDFDownloader(vacancy_id=str(recruiter_params.application_number),
                                           ...
                                           candidate_surname=applicant.title)
